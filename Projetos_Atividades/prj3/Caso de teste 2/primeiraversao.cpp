...
//Para alterar arquivos, vá para a linha 211
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int lerProdutos(const char* filename, Produto** produtosPtr) {
    ...
    int capacidade = 10; // Começamos com espaço para 10 produtos
    ...
    while (fscanf(file, "%d;%d;%d\n", &produtos[i].codProduto, &produtos[i].inicioEstoque, &produtos[i].minEstoque) != EOF) {
        i++;
        if (i >= capacidade) { // Precisamos de mais espaço
            capacidade *= 2;
...
int lerVendas(const char* filename, Venda** vendasPtr) {
    ...
    int capacidade = 10; // Começamos com espaço para 10 vendas
...
void saidaDivergencia(const char* filename, Produto* produtos, int numProdutos, Venda* vendas, int numVendas) {
    ...
    for (int i = 0; i < numVendas; i++) {
        ...
        } else if (vendas[i].sitVendas == 135) {
            fprintf(file, "Linha %d - Venda cancelada\n", i + 1);
        } else if (vendas[i].sitVendas == 190) {
...
int main() {
    ...
    int numProdutos = lerProdutos("c2_produtos.txt", &produtos);
    int numVendas = lerVendas("c2_vendas.txt", &vendas);
